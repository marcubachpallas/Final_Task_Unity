...
public class BRGEditor : EditorWindow
{
    ...
    static void CreateSelection(int layerIndex, int tagindex)
    {
        ...
        GameObject[] tagSelection = MVDUtils.FindGameObjectsWithTag(miscValues.allTags[tagindex]);
            ...
            case 0:
                {
                    ...
                    {
                        Undo.RegisterCompleteObjectUndo(Selection.objects[i], "rename " + Selection.objects[i]);
    ...
    {
        if (rootEditor) DestroyImmediate(rootEditor);
    }
}