...
public class ClickSpawn : MonoBehaviour
{
    ...
    void OnScene(SceneView scene)
    {
        ...
        if (Physics.Raycast(ray, out hit, float.PositiveInfinity, 1 << layerIndex))
        {
            ...
            transform.position = hit.point + position;
            transform.rotation = new_rotation * Quaternion.Euler(rotation);
            ...
            if (e.type == EventType.MouseDown && e.button == 2)
            {
                ...
                go.transform.rotation = new_rotation * Quaternion.Euler(rotation);
                ...
                Undo.RegisterCreatedObjectUndo(go, "Undo Spawn: " + go.name);
